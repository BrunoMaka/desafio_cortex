...
class Crawler(Tools):    
    def get_infos(self, max_collect):
        for doc in DOCS:
            ...
            while len(self.webdriver.find_elements(*L_NEXT_PAGE)) > 0:
                ...
                self.open_tab()                 
                self.get_data(links, max_collect)
                self.close_tab()
                self.click_in_element(L_NEXT_PAGE)
                time.sleep(2)
                if len(self.consulta['processos']) >= max_collect:
                    break
    ...
    def get_data(self, links, max_collect):              
        for link in links:                                     
            ...
            #print(f'Finalizado: {processo["numeroProcessoUnico"]}')
            self.consulta['processos'].append(processo) 
            print(len(self.consulta["processos"])) 
            if len(self.consulta['processos']) >= max_collect:
                break
